...
public class SignService {
	...
	private final SignSettingsService signSettingsService;

	public SignService(SignSettingsService signSettingsService) {
		this.signSettingsService = signSettingsService;
	}

	public void reconcileGuildSign(Guild guild) {

		Optional<SignSettings> maybeConfig = signSettingsService.getForSignChange(guild.getIdLong());
		if(maybeConfig.isEmpty())
			return;

		SignSettings config = maybeConfig.get();
		...
		String targetName = hasAnyHumanInAudioChannel(guild) ? config.getLabelOpen() : config.getLabelClosed();
